...
from .mongo_serializers import test_catalogSerializer
...
def test_catalog_list_create(request):
    if request.method == "GET":
        q = dict(request.query_params)
        docs = [fix_id(d) for d in col.find(q)]
        return Response(docs)

    serializer = test_catalogSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def test_catalog_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = test_catalogSerializer(
