...
from .mongo_serializers import lab_order_eventsSerializer

col = db["lab_order_events"]
...
def lab_order_events_list_create(request):
    if request.method == "GET":
        q = dict(request.query_params)
        docs = [fix_id(d) for d in col.find(q)]
        return Response(docs)

    serializer = lab_order_eventsSerializer(data=request.data)
...
@permission_classes([IsAuthenticated])
def lab_order_events_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = lab_order_eventsSerializer(
